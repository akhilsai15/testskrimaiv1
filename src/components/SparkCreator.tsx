...
import { X, Type, Camera, Image as ImageIcon, Sparkles, Wand2, Plus, Globe, Users } from 'lucide-react';
...
export function SparkCreator({ isOpen, onClose, onPost }: SparkCreatorProps) {
  const sparkThemes = [
  ...
  const renderContent = () => {
                  ...
                  {sparkThemes.map(theme => (
                    <button 
                      ...
                      style={{ 
                        ...
                        transform: selectedTheme.id === theme.id ? "scale(1.2)" : "scale(1)"
                      }} 
                    />
                  ))}
                </div>
    ...
    return (
      <div className="flex-1 flex flex-col bg-[#0A0A0A] w-full relative h-[100dvh]">
        ...
        {/* Media Preview */}
        <div className={`w-full relative bg-black flex items-center justify-center overflow-hidden shrink-0 ${selectedMedia?.type === 'video' ? 'h-[55vh]' : 'h-[60vh]'}`}>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pb-safe">
          ...
          <div className="w-full overflow-x-auto no-scrollbar py-3 px-4 flex gap-4 border-b border-white/5">
            ...
            <button className={`flex flex-col items-center gap-1 shrink-0 group relative`}>
              <div className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all bg-white/5 border border-white/10 group-hover:bg-white/10 text-xl`}>
                {mood === '🔥 Trending' ? '😊' : mood}
              </div>
              <span className="text-[10px] font-bold text-white/60">Mood</span>
            </button>
            ...
            {selectedMedia?.type === 'video' ? (
             ...
             <>
                <button className={`flex flex-col items-center gap-1 shrink-0 group`}>
                  ...
                  <span className="text-[10px] font-bold text-white/60">Filter</span>
                </button>
                <button className={`flex flex-col items-center gap-1 shrink-0 group`}>
                  <div className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all bg-white/5 border border-white/10 group-hover:bg-white/10`}>
                    <span className="text-xl">🔆</span>
                  </div>
                  <span className="text-[10px] font-bold text-white/60">Adjust</span>
                </button>
             </>
            )}
          </div>
          ...
          <div className="flex flex-col p-4 gap-6">
            ...
            <button
              onClick={handlePost}
              className="mt-2 w-full bg-gradient-to-r from-[#B026FF] to-[#00F0FF] text-white font-bold py-4 rounded-2xl shadow-[0_0_20px_rgba(176,38,255,0.4)] hover:shadow-[0_0_30px_rgba(176,38,255,0.6)] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 mb-10"
            >
                POST SPARK ⚡
            </button>
          </div>
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/95 backdrop-blur-md sm:p-4 perspective-[1000px]">
        <motion.div
           ...
           className="w-full h-full sm:w-[420px] sm:h-[800px] sm:max-h-[90vh] sm:rounded-[40px] bg-black text-white flex flex-col overflow-hidden sm:border sm:border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative z-10"
        >
