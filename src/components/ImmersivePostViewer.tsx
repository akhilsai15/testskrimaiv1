...
export function ImmersivePostViewer({ initialIndex, type, urls, user, onClose }: ImmersivePostViewerProps) {
      ...
      <div className="absolute top-0 w-full p-4 flex justify-between items-center z-30">
        ...
        >
          <div className="w-10 h-10 rounded-full p-[2px] bg-gradient-to-br from-[#B026FF] to-[#00F0FF] group-hover:scale-110 transition-transform">
            <img src={user?.avatar || 'https://i.pravatar.cc/150'} alt="Avatar" className="w-full h-full rounded-full border-2 border-black object-cover" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <p className="text-sm font-bold text-white drop-shadow-md group-hover:underline">{user?.username || 'user'}</p>
        ...
        <div className="px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-md border flex items-center gap-1.5 shadow-lg"
          ...
          {(!type || type === 'post' || type === 'image') && <><span className="text-sm drop-shadow-md">⚡</span> <span className="text-xs font-bold text-[#B026FF]">Pulse Post</span></>}
          {(type === 'reel' || type === 'video' || type === 'vibe') && <><span className="text-sm drop-shadow-md">🎬</span> <span className="text-xs font-bold text-[#00F0FF]">Vibe</span></>}
          {type === 'story' && <><span className="text-sm drop-shadow-md">✨</span> <span className="text-xs font-bold text-[#FF2D87]">Spark</span></>}
          {type === 'saved' && <><span className="text-sm drop-shadow-md grayscale opacity-80">🔖</span> <span className="text-xs font-bold text-gray-400">Saved</span></>}
      ...
      <div className="relative z-10 w-full h-[65dvh] max-h-[800px] flex items-center justify-center">
        <AnimatePresence mode="popLayout">
          <motion.div
            ...
            className={`relative w-[90%] max-w-[450px] shadow-[0_20px_60px_rgba(176,38,255,0.4)] cursor-pointer touch-none ${type === 'vibe' ? 'aspect-[9/16]' : 'aspect-square'} rounded-[24px] overflow-hidden`}
